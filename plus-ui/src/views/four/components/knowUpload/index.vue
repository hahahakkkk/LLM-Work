<template>
  <div class="upload-file">
    <el-upload
      ref="fileUploadRef"
      multiple
      :action="uploadFileUrl + '?configKey=' + configKey + '&knowledgeType=' + knowledgeType"
      ...
      :disabled="!knowledgeType"
    >
      <!-- 上传按钮 -->
      <el-button type="primary" :disabled="!knowledgeType">选取文件</el-button>
    </el-upload>
    <!-- 上传提示 -->
...
const props = defineProps({
  ...
  // 知识类型
  knowledgeType: propTypes.string.def(''),
  // 是否显示文件列表
...
const handleBeforeUpload = (file: any) => {
  if (!props.knowledgeType) {
    proxy?.$modal.msgError('请先选择知识类型');
    return false;
  }
  // 校检文件类型

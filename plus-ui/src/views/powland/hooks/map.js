...
import TileLayer from 'ol/layer/Tile'; //吉林一号使用
// import TileLayer from 'ol/layer/WebGLTile.js';  //geoserver使用
...
/**
 * 初始化地图并挂载到指定容器
 * @param {string} [targetId='map'] 容器 DOM id
 * @returns {ol/Map} map 实例
 */
function initMap(targetId = 'map') {
  console.log(map);
  // 如果已经初始化，则直接返回现有 map（防止重复创建）
  // if (map) {
  //   // 如果已经挂载在不同容器，直接返回已有实例；若需要切换容器可扩展
  //   return map;
  // }
...
const landLevel = {
   ...
   * @param {*} landJson  地块数据
   */
  addLands: function (landJson) {
    this.landSource.clear(true);
    this.landSource.addFeatures(new GeoJSON().readFeatures(landJson));
  },

  /**
   * 地块图层初始化
   * @param {*} levelColors 等级色彩
   */
  init: function (levelColors) {
    this.landSource = new VectorSource({ wrapX: false });
    const landLayer = new VectorLayer({
      source: this.landSource,
      style: (f) => {
        const level = parseInt(f.get('landLevel'));
        let lcolor = '#ff0000aa';
        if (level) {
          lcolor = levelColors[level - 1];
        }
        return new Style({
          fill: new Fill({
            color: `${lcolor}`
  ...
  locate(id) {
    const view = map.getView();
    const allFeature = this.landSource.getFeatures();
    let feature = null;
    for (var i = 0; i < allFeature.length; i++) {
      if (allFeature[i].get('landId') == id) {
...
export function useMap() {
  return { initMap, resetView, landUnit, locate, collectPoint, gcj02ToG84, landLevel };
}

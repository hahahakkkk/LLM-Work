// 整体布局参数调整（适配上下布局：图片+条形码）
var offsetX = 0;
var offsetY = 0;
// 宽度与img.js/barcode.js保持一致，高度增加以容纳上下两部分
var width = 70;
var height = 80;
var rotate = 0;
// 整体边距（左右/上下）
...
export const combinationPrintData = {
  ...
  elements: [
    // 第一个元素：图片（上半部分）
    {
      type: 'image',
      json: {
        x: marginX + offsetX, // 水平位置：左边距+偏移
        y: marginY + offsetY, // 垂直位置：上边距+偏移（顶部）
        height: imgHeight, // 图片高度
        width: imgWidth, // 图片宽度
        rotate: 0,
        imageProcessingType: 0,
        imageProcessingValue: 127,
        // 注：这里先保留占位，实际使用时会替换为动态的二维码Base64（和img.js逻辑一致）
        imageData: ''
      }
    },
    // 第二个元素：条形码（下半部分）
    {
      type: 'barCode',
      json: {
        x: marginX + offsetX, // 水平位置：和图片对齐（左边距+偏移）
        // 垂直位置：图片底部 + 间距 + 上边距
        y: marginY + offsetY + imgHeight + gapBetweenImgBarcode,
        height: barCodeHeight, // 条形码高度
        width: barCodeWidth, // 条形码宽度（和图片等宽）
        value: '12345678', // 占位值，实际使用时替换为动态溯源码
        codeType: 20, // 条形码类型（和barcode.js保持一致）
        rotate: 0,
        fontSize: 3.2, // 条形码下方文字大小
        textHeight: 3.2, // 文字高度
        textPosition: 0 // 文字位置（0=下方，1=上方，2=无）
      }
    }
  ]
};

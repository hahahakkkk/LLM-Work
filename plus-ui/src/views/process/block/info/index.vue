...
import { ref, reactive, getCurrentInstance, ComponentInternalInstance, toRefs, computed, onMounted } from 'vue';
...
onMounted(async () => {
  ...
  // 添加详细的字典数据检查
  console.log('=== 字典数据检查 ===');
  console.log('crop_variety:', crop_variety);
  console.log('crop_variety.value:', crop_variety?.value);
  console.log('irrigation_method:', irrigation_method);
  console.log('irrigation_method.value:', irrigation_method?.value);
  console.log('use_method:', use_method);
  console.log('use_method.value:', use_method?.value);

  // 检查 proxy.useDict 方法
  console.log('proxy:', proxy);
  if (proxy?.useDict) {
    console.log('useDict method exists');
  }

  // 读取路由参数并自动填充+搜索
  const traceCodeFromRoute = route.query.traceCode as string;
  if (traceCodeFromRoute) {
    // 填充到输入框
    searchParams.traceCode = traceCodeFromRoute;
    // 自动触发搜索
    await handleSearch();
  }
});

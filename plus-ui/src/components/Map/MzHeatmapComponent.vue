<template>
  <div class="mz-heatmap-wrapper">
    ...
    <div v-if="useDraggablePanels" class="panel-toggle-group">
      <button v-if="showControls && !controlsPanel.visible" @click="showPanelById('controls')" class="toggle-btn" title="显示控制面板">🎛️</button>
      <button v-if="showStats && !statsPanel.visible" @click="showPanelById('stats')" class="toggle-btn" title="显示统计面板">📊</button>
    </div>

    <!-- 控制面板插槽 -->
    <div v-if="showControls && !useDraggablePanels" class="heatmap-controls">
      <slot name="controls" :updateConfig="updateConfig" :toggleBoundary="toggleBoundary">
        <!-- 默认控制面板 -->
        <div class="default-controls">
          ...
          <div class="control-group">
            <label>显示边界</label>
            <input type="checkbox" v-model="boundaryVisible" @change="toggleBoundary" />
          </div>
        </div>
      </slot>
    </div>
    ...
    >
      <div class="panel-header" @mousedown="startDrag($event, 'controls')">
        ...
        <div class="panel-controls">
          <button @click="togglePanel('controls')" class="panel-btn collapse-btn" :class="{ collapsed: !controlsPanel.expanded }">▼</button>
          <button @click="closePanel('controls')" class="panel-btn close-btn">✕</button>
        </div>
      </div>
    ...
    >
      <div class="panel-header" @mousedown="startDrag($event, 'stats')">
        ...
        <div class="panel-controls">
          <button @click="togglePanel('stats')" class="panel-btn collapse-btn" :class="{ collapsed: !statsPanel.expanded }">▼</button>
          <button @click="closePanel('stats')" class="panel-btn close-btn">✕</button>
        </div>
      </div>
...
import MzHeatmap, { type HeatPoint, type HeatmapConfig, type BoundaryData } from './MzHeatmap';
import { StyleProfile } from './MzMapBase';

...
const isReadyForRecognition = computed(() => {
  return selectedBase.value && selectedModelId.value && images.value.length > 0;
});
...
const getOptions = async () => {
  try {
    ...
    // 过滤出模型名称包含"成熟度"的模型
    options.value = res.rows.filter((model) => model.modelName.includes('成熟度'));

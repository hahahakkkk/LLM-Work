  ...
  <div class="p-2">
        ...
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <!-- <el-form-item label="基地名称" prop="baseName">
              <el-select v-model="queryParams.baseName" placeholder="请选择基地名称" clearable @change="onSearchBaseChange">
                <el-option
                  v-for="item in baseDict"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
            ...
            <el-form-item label="地块编号" prop="plotId">
              <el-select v-model="queryParams.plotId" placeholder="请选择地块编号">
    ...
    <el-card shadow="never">
      <template #header>
        <el-row :gutter="10" class="mb8">
          <el-col :span="1.5">
            <el-button v-hasPermi="['growth:diagnosis:add']" type="primary" plain icon="Plus" @click="handleAdd">新增</el-button>
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['growth:diagnosis:edit']" type="success" plain icon="Edit" :disabled="single" @click="handleUpdate()"
              >修改</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['growth:diagnosis:remove']" type="danger" plain icon="Delete" :disabled="multiple" @click="handleDelete()"
              >删除</el-button
            >
          </el-col>
          <el-col :span="1.5">
            <el-button v-hasPermi="['growth:diagnosis:export']" type="warning" plain icon="Download" @click="handleExport">导出</el-button>
          </el-col>
          <!--          <el-col :span="1.5">-->
      ...
      <el-table v-loading="loading" :data="diagnosisList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="诊断时间" align="center" prop="diagnosisTime" width="100">
          <template #default="scope">
        ...
        <el-table-column label="预测状态" align="center" prop="isPredicted">
          <template #default="scope">
            <el-tag :type="scope.row.isPredicted === 1 ? 'success' : 'warning'">
              {{ scope.row.isPredicted === 1 ? '已完成' : '预测中' }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            <el-tooltip content="查看" placement="top">
              <el-button v-hasPermi="['growth:diagnosis:view']" link type="primary" icon="View" @click="handleView(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="修改" placement="top">
              <el-button v-hasPermi="['growth:diagnosis:edit']" link type="primary" icon="Edit" @click="handleUpdate(scope.row)"></el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button v-hasPermi="['growth:diagnosis:remove']" link type="primary" icon="Delete" @click="handleDelete(scope.row)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
    ...
    <el-dialog v-model="addDialog.visible" :title="addDialog.title" width="500px" append-to-body>
      <el-form ref="diagnosisFormRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="基地名称" prop="baseName">
          <el-select v-model="form.baseName" placeholder="请选择基地" style="width: 100%" @change="onBaseChange">
        ...
        <!-- <el-form-item label="诊断模型" prop="diagnosisModel">
          <el-select v-model="form.diagnosisModel" placeholder="请选择诊断模型">
            <el-option label="RGB图像智能诊断模型" value="RGB图像智能诊断模型" />
            <el-option label="LAI_SPAD诊断模型" value="LAI_SPAD诊断模型" />
            <el-option label="长势分析模型" value="长势分析模型" />
          </el-select>
        </el-form-item> -->
        <el-form-item label="长势等级" prop="growthLevel">
          <el-select v-model="form.growthLevel" placeholder="请选择长势等级">
            <el-option label="良好" value="良好" />
            <el-option label="正常" value="正常" />
            <el-option label="较差" value="较差" />
          </el-select>
        </el-form-item>
        <!-- <el-form-item label="预测状态" prop="isPredicted">
          <el-select v-model="form.isPredicted" placeholder="请选择预测状态">
            <el-option label="预测中" :value="0" />
...
import { listDiagnosis, getDiagnosis, delDiagnosis, addDiagnosis, updateDiagnosis } from './api';
import { DiagnosisVO, DiagnosisQuery, DiagnosisForm } from './api/types';
...
const handleAdd = () => {
  ...
  if (baseDict.value && baseDict.value.length > 0) {
    form.value.baseName = baseDict.value[0].value;
    // 触发基地选择变化，加载对应地块
    onBaseChange();
  }
};
...
const onBaseChange = async () => {
  ...
  if (form.value.baseName) {
    try {
      const res = await fetchFarmerLands({ baseId: form.value.baseName });
      landDict.value =
        res.rows?.map((item) => ({
          value: String(item.landId),
          label: String(item.landCode)
...
// 处理搜索表单中的基地选择变化
const onSearchBaseChange = async () => {
  // 当搜索表单中的基地改变时，清空地块选择
  queryParams.value.plotId = undefined;
  ...
  if (queryParams.value.baseName) {
    try {
      const res = await fetchFarmerLands({ baseId: queryParams.value.baseName });
      landDict.value =
        res.rows?.map((item) => ({
          value: String(item.landId),
          label: String(item.landCode)
      ...
      console.error('获取搜索基地地块失败:', error);
      landDict.value = [];
    }
  } else {
    // 如果清空了基地选择，则重新加载所有地块

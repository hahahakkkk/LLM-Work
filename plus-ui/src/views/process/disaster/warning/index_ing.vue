<template>
  <div class="disaster-map-container">
    ...
    <div class="right-panel">
      <el-card>
        <el-select v-model="selectedDisaster" @change="handleDisasterChange" style="width: 100%; margin-bottom: 10px">
...
//   { id: 1, name: '候家沟基地', village: '候家沟', type: '智慧引领种植基地', lng: 110.37847, lat: 37.76523 },
...
//   drought: { '轻旱': 'yellow', '中旱': 'orange', '重旱': 'red', '极旱': 'darkred' },
//   flood: { '一般洪涝': 'lightblue', '较大洪涝': 'blue', '重大洪涝': 'purple', '特别重大洪涝': 'darkpurple' },
//   hail: { '轻度冰雹': 'lightgray', '中度冰雹': 'gray', '重度冰雹': 'darkgray', '特重冰雹': 'black' }
// };
...
//     const matching = warningList.value.find(row => row.region?.includes(base.village) && row.disasterType === selectedDisaster.value);
...
//     if (matching) {
//       level = matching.warningLevel;
//       content = matching.warningContent;
//       color = disasterColorMap[selectedDisaster.value]?.[level] || 'gray';
//     }
...
//   const base = baseCoordinates.find(b => row.region?.includes(b.village));
//   if (base) {
//     selectedDisaster.value = row.disasterType;
//     handleDisasterTypeChange();
//     setTimeout(() => focusMarkerByBaseId(base.id), 300);
//   } else {
...
//   await getList();
// });
import { initMap, updateMarkers, focusOnBase } from '../hooks/map';

const selectedDisaster = ref('all');
const warningList = ref([]);

// 加载预警数据
const loadWarnings = async () => {
  const res = await listWarning({
    disasterType: selectedDisaster.value === 'all' ? undefined : selectedDisaster.value
  });
  warningList.value = res.rows;
  updateMarkers(res.rows);
};
...
const handleRowClick = (row) => {
  const base = baseStations.find((b) => row.region.includes(b.village.replace('村', '')));
  if (base) focusOnBase(base.id);
};

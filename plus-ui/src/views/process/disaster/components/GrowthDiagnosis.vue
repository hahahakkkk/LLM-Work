...
const setDefaultValues = async (values: any) => {
  ...
  if (values.baseId) {
    ...
    if (selectedBase) {
      form.value.baseName = selectedBase.label;
    }
  }
  ...
  // 设置生长时期
  if (values.growthPeriod) {
    form.value.growthPeriod = values.growthPeriod;
  } else if (growth_period.value && growth_period.value.length > 0) {
    // 如果没有传递生长时期，使用第一个选项作为默认值
    form.value.growthPeriod = growth_period.value[0].value;
  }

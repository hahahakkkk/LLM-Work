<template>
  <div class="p-2">
      ...
      <el-aside width="75%">
        <el-descriptions class="margin-top" title="地块基本信息" :column="3" :border="true">
          <el-descriptions-item>
            ...
            <el-select v-model="data.landId" placeholder="请选择种植地块" filterable @change="onLandSelected">
              <el-option v-for="dict in landList" :key="dict.label" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-descriptions-item>
          <el-descriptions-item>
            <template #label>
          ...
          <el-descriptions-item>
            ...
            <div class="value-contain">
              ...
              ><el-tag size="small" type="success">亩</el-tag>
            </div>
          </el-descriptions-item>
          <el-descriptions-item>
            <template #label>
          ...
          <el-descriptions-item>
            ...
            <div class="value-contain">
              <span>{{ data.soilDensity }}</span
              ><el-tag type="success">g/cm<sup>3</sup></el-tag>
            </div>
          </el-descriptions-item>
          <el-descriptions-item>
            <template #label>
    ...
    <el-container style="background-color: #eef1f6">
      <div style="margin: 20px; width: 100%; display: flex; justify-content: flex-end">
        <el-button type="warning" size="large" @click="formReset" :icon="RefreshLeft">重制配方</el-button>
        <el-button size="large" @click="goToHistory" :icon="List">配方历史</el-button>
      </div>
    </el-container>
...
import { Download, EditPen, Edit, Iphone, Location, OfficeBuilding, Tickets, User, View, RefreshLeft, List } from '@element-plus/icons-vue';
import html2canvas from 'html2canvas';
import jsPDF from 'jspdf';
import { pointInLand } from '@/views/powland/api/point';
import { fertDictQuery, organicDictQuery, landDictQuery } from '../api/tableDict';
...
function formReset() {
  ...
  Object.assign(data, data_init);
  fertilizer_organic_data.baseForm.records = [
    {
      id: '3',
      volumn: 120
    }
...
function goToHistory() {
  router.push('/baseManage/situation/fertilizationHistory');
}
...
async function getLandList() {
  ...
  landList.value = res.rows.filter((it) => it.isValid === 1).sort((a, b) => a.label.localeCompare(b.label, 'zh-CN'));
}
/**
 * 地块选择
 */
function onLandSelected() {
  loadData(data.landId);
}

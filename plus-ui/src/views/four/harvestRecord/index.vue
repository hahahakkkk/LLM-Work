  ...
  <div class="p-2">
      ...
      <div v-show="showSearch" class="mb-[10px]">
        <el-card shadow="hover">
          <el-form ref="queryFormRef" :model="queryParams" :inline="true">
            <!--<el-form-item label="所属基地" prop="baseId" style="width: 220px">
            ...
            <!--<el-form-item label="种植地块" prop="landId" style="width: 220px">
              <el-select v-model="queryParams.landId" placeholder="请选择种植地块" clearable>
    ...
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="sowHarvestList" @selection-change="handleSelectionChange">
        ...
        <el-table-column label="年份" align="center" prop="shYear" width="100" />
        <el-table-column label="所属基地" align="center" prop="baseId">
          <template #default="scope">
            <dict-tag :options="baseDict" :value="scope.row.baseId" />
          </template>
        </el-table-column>
        <el-table-column label="种植地块" align="center" prop="landId">
          <template #default="scope">
        ...
        <el-table-column label="种植谷子品种" align="center" prop="cropVariety">
          <template #default="scope">
        ...
        <!-- 新增合并后的播种人列 -->
        <el-table-column label="播种人" align="center" prop="mergedSower" v-if="false">
          <template #default="scope">
        ...
        <!-- 新增合并后的收割人列 -->
        <el-table-column label="收割人" align="center" prop="mergedHarvester" v-if="false">
          <template #default="scope">
            <div v-if="scope.row.harvestMethod === '1'">
              <!-- 农户信息显示 -->
              <template v-if="getFarmerName(scope.row.harvesterFarmer)">
                <el-tag style="color: #409eff; font-weight: 500">
                  {{ getFarmerName(scope.row.harvesterFarmer) }}
                </el-tag>
              </template>
              <span v-else-if="scope.row.harvesterFarmer == null" style="color: #909399"></span>
              <span v-else style="color: #f56c6c">无效农户ID：{{ scope.row.harvesterFarmer }}</span>
            </div>
            <!-- 非农户信息显示 -->
            <div v-else-if="scope.row.harvestMethod === '0'" class="harvester-ext">
              <el-tag type="info">
                {{ scope.row.harvesterExt || '未命名收割人' }}
              </el-tag>
            </div>
            <span v-else style="color: #909399">N/A</span>
          </template>
        ...
        <!--<el-table-column label="操作" align="center" class-name="small-padding fixed-width">
          <template #default="scope">
            ...
            <el-tooltip content="收割" placement="top">
              <el-button link type="primary" icon="Check" @click="handleUpdate2(scope.row)" v-hasPermi="['mz_base:sowHarvest:edit']"></el-button>
            </el-tooltip>
      ...
      <el-form ref="sowHarvestFormRef" :model="form" :rules="rules" label-width="110px">
        <e-skeleton :loading="columnLoading" v-if="columnLoading">
          ...
          <!--<el-form-item label="种植地块" prop="landId" v-if=display1>
            <el-select v-model="form.landId" placeholder="请选择种植地块" clearable>
          ...
          <!--<el-form-item
            v-if="form.seederType === '0'"
            label="播种人"
            prop="farmerId"
            key="farmer-select">
            <el-select v-model="form.farmerId" placeholder="请输入播种人" clearable>
        ...
        <e-skeleton :loading="columnLoading2" v-if="columnLoading2">
          ...
          <!-- 农户下拉选择（当harvestMethod=1时显示） -->
          <!--<el-form-item
            v-if="form.harvestMethod === '1'"
            label="收割人"
            prop="harvesterFarmer"
            key="farmer-select">
            <el-select v-model="form.harvesterFarmer" placeholder="请选择收割人" clearable>
                <el-option v-for="dict in farmerLandDict"  :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>-->
          <!-- 非农户输入（当seederType=0时显示） -->
          <!--<el-form-item
            v-if="form.harvestMethod === '0'"
            label="收割人"
            prop="harvesterExt"
            key="non-farmer-input">
            <el-input v-model="form.harvesterExt" placeholder="请输入非农户名称" />
          </el-form-item>
          <el-form-item label="产量(公斤)" prop="yeild">
          ...
          <!-- 农户下拉选择（当harvesterType=0时显示） -->
          <!--<el-form-item
            v-if="form.harvesterType === '0'"
            label="收割人"
            prop="harvesterFarmer"
            key="harvester-farmer-select">
            <el-select v-model="form.harvesterFarmer" placeholder="请选择收割人" clearable>
            <el-option v-for="dict in farmerLandDict" :label="dict.label" :value="dict.value" />
          </el-select>
        </el-form-item>-->
          <el-form-item v-if="form.harvesterType === '0'" label="收割人" prop="harvesterFarmer" key="harvester-farmer-select">
            <el-select v-model="form.harvesterFarmer" placeholder="请选择收割人" clearable filterable>
              <el-option v-for="dict in sortedfarmerBaseDict" :label="dict.label" :value="dict.value" />
            </el-select>
          </el-form-item>
          <!-- 非农户输入（当harvesterType=1时显示） -->
          <el-form-item v-if="form.harvesterType === '1'" label="收割人" prop="harvesterExt" key="harvester-ext-input">
            <el-input v-model="form.harvesterExt" placeholder="请输入非农户名称" />
          </el-form-item>
...
import { listSowHarvest, getSowHarvest, delSowHarvest, addSowHarvest, updateSowHarvest } from '@/views/mz_base/api/sowHarvest';
import { SowHarvestVO, SowHarvestQuery, SowHarvestForm } from '@/views/mz_base/api/sowHarvest/types';
import { landDictQuery, farmerDictQuery, baseDictQuery, farmerBaseDictQuery, landAreaDictQuery } from '@/views/mz_base/api/tableDict';
...
/*const onBaseChange = (baseId)=>{
  ...
  if(currentBaseLabel.value === '侯家沟'){
    ...
    filteredLandDict.value = landDict.value.filter(land =>
      land.label.includes('侯家沟') && !land.label.includes('南')
    );
  } else {
    // 筛选地块：地块标签以基地标签开头
    filteredLandDict.value = landDict.value.filter(land =>
      land.label.startsWith(currentBaseLabel.value)
    );
  }
...
const calculateTotalYield = (row) => {
  if (!row.yeild || !row.landId) return 0;
...
/*const getDicts = async () => {
  ...
  res = await baseDictQuery();
  baseDict.value = res.rows.sort((a, b) => b.label.localeCompare(a.label, 'zh-CN'));
